...
export default function Index({ biotas, auth }) {
    ...
    const handleSearch = () => {
        setIsSearching(true);
        const filteredItems = biotas.filter((biota) => {
            return biota.name.toLowerCase().includes(searchTitle.toLowerCase());
        });
                    ...
                    filteredBiotas.length > 0 ? (
                        <>
                            <div className="grid sm:grid-cols-2 md:grid-cols-3 md:gap-x-8 lg:grid-cols-4 xl:grid-cols-5">
                                {filteredBiotas.map((biota, index) => (
                                    <div
                                        key={index}
                        ...
                        <div className="flex flex-col justify-center items-center p-4">
                            <h1 className="text-2xl md:text-5xl leading-5 font-bold text-gray-900">
                                Biota yang kamu cari tidak ada
                            </h1>
                        </div>
                    )
                ) : (
